...
export default function DashboardLayout({ children }) {
  ...
  return (
    <div className={`flex min-h-screen w-full flex-col md:h-screen md:flex-row md:overflow-hidden ${theme.main}`}>
      <aside className={`w-full shrink-0 md:flex md:h-full md:w-64 md:flex-col ${theme.sidebar}`}>
        <div className={`h-1 ${theme.accentBar}`} />

        <div className={`px-4 py-3 sm:px-5 sm:py-4 md:px-6 md:py-5 ${theme.sidebarBorder}`}>
          ...
          <p className={`mt-1 text-[10px] font-bold uppercase tracking-widest sm:mt-2 sm:text-[11px] ${theme.panelLabel}`}>
            {role} Panel
          </p>
        </div>
        ...
        <div className={`hidden border-t p-4 md:block ${theme.sidebarBorder.replace("border-b", "")}`}>
          <button
            type="button"
            onClick={handleLogout}
      ...
      <main className="min-h-0 min-w-0 w-full flex-1 overflow-x-hidden overflow-y-auto">{children}</main>
    </div>
  );
}
