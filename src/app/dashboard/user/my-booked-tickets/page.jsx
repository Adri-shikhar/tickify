...
// My Booked Tickets page (route: /dashboard/user/my-booked-tickets)
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-4xl flex-col gap-6 bg-gray-50/50 p-6 md:p-8">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">Your confirmed trips.</p>
      </div>
      ...
      {!loading && !error && !bookings.length && (
        <p className="rounded-2xl border bg-white py-12 text-center text-sm text-gray-400">No tickets booked yet.</p>
      )}

      <div className="flex flex-col gap-4">
        {bookings.map(({ _id, ticketTitle, departureDateTime, bookedAt, seatsBooked, totalPrice }) => (
          <Card key={_id} className="flex flex-col gap-4 border border-gray-100 p-5 shadow-md">
            <div className="flex items-start justify-between gap-2">
              <h2 className="text-lg font-bold text-gray-900">{ticketTitle}</h2>
              <span className="rounded border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[10px] font-black uppercase text-emerald-600">Confirmed</span>
            </div>

            <div className="flex justify-between rounded-xl bg-gray-50 p-3 text-xs text-gray-600">
