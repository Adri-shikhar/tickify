...
export default function MyBookedTicketsPage() {
  ...
  const { data: bookings = [], error, loading } = useSessionData(session?.user?.id, getUserBookings, "bookings");

  // Inline configuration to resolve status badge styling mappings
  const getBadgeMeta = (status) => {
    if (status === "pay") return { text: "Approved", cls: "bg-blue-50 border-blue-200 text-blue-600" };
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-4xl flex-col gap-6 bg-gray-50/50 p-6 md:p-8 isolate">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">Track and process payments for your active trip reservations.</p>
      </div>

      {loading && <p className="animate-pulse text-sm text-gray-500 font-medium">Loading your bookings...</p>}
      ...
      <div className="flex flex-col gap-4">
        {bookings.map(({ _id, ticketTitle, departureDateTime, bookedAt, seatsBooked, totalPrice, status }) => {
          const badge = getBadgeMeta(status);

          return (
            <Card key={_id} className="flex flex-col gap-4 border border-gray-100 p-5 shadow-md transform-gpu backface-hidden bg-white rounded-2xl">
              
              {/* Header Title & Dynamic Workflow Badges */}
              <div className="flex items-start justify-between gap-4">
                <h2 className="text-lg font-bold text-gray-900 tracking-tight line-clamp-1">{ticketTitle}</h2>
                <span className={`rounded-md border px-2.5 py-0.5 text-[10px] font-black uppercase tracking-wider shrink-0 ${badge.cls}`}>
                  {badge.text}
                </span>
              </div>

              {/* Travel Schedule Info block */}
